...
- **`test/` folder at the repository root:** `test/domain` (`*.spec`, unit — the test target), `test/application` (`*.test`, integration — the test target), plus support-only folders `test/fakes`, `test/stubs`, `test/helpers` (setup, truncate, container factory, builders like `givenOrder`/`givenTier`).
...
- camelCase for methods, functions and variables; PascalCase for classes and interfaces; kebab-case for files and directories.
